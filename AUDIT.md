...
- **3.5 frontend hardening** (mostly done), **3.6 design-doc freeze** — **done (2026-08-22)**: `Final_System_Design.md` weights corrected to the benchmark-verified 20% BM25 / 80% semantic linear reorder (was the stale 40/30/15/10/5 table), reranker marked optional/off-by-default in both diagrams + component table, dual-path assistant and pinned answers referenced. Tech-stack/folder docs already carried the dictionary-entity-extraction as-built notes.
